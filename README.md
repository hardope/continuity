...
| Media control | Android can remote-control Play/Pause/Next/Previous on a connected **macOS** device (global media-key injection — controls whatever currently owns Now Playing, no per-app integration needed) and shows its album art, title, and artist (read from macOS's private MediaRemote framework, pushed to the phone whenever it changes). Windows/Linux aren't wired up yet. See [`docs/protocol.md`](docs/protocol.md#media-control). |
