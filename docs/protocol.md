...
Fixed by giving `continuityd` a real, directly-linked GTK3 implementation of `rfd::MessageDialog` on Linux (`rfd = { version = "0.15", features = ["gtk3"] }`, additive with the default `xdg-portal` feature used for file dialogs elsewhere) — GTK is already a hard runtime dependency on Linux regardless, since `tray-icon` needs it for the tray icon itself, so this adds no new system requirement. `zenity` is also declared as an explicit `.deb` dependency (`depends = ["$auto", "zenity"]`) as a belt-and-suspenders fallback, since `cargo-deb`'s `"$auto"` resolves dependencies via `ldd` against the built binary and has no way to see a binary that's shelled out to via `std::process::Command` rather than dynamically linked.

**Unverified** — no Linux machine available in this development environment; this is code/dependency-research-based (`rfd`'s own source and Cargo feature docs), not confirmed against a real Linux desktop.
